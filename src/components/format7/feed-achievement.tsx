...
import { ChevronDown, ChevronUp, Clock, Heart, MessageSquare, Repeat2, MoreVertical, Trophy } from 'lucide-react';
import { useContainedConfetti } from '../../hooks/useContainedConfetti';

interface FeedAchievementProps {
...
const FeedAchievement: React.FC<FeedAchievementProps> = ({ entry, showUserProfile = true }) => {
  ...
  const isAchievement = entry.entry_metadata.type === 'achievement';
  ...
  const handleAchievementHover = () => {
    if (isAchievement && cardRef.current) {
      const now = Date.now();
  ...
  return (
    <div className="w-full">
      {/* Entry Title */}
      <div className="mb-6">
        <h3 className="text-sm font-semibold text-gray-500 mb-2">Feed View: Achievement Celebration</h3>
      </div>

      {/* Card with hover shadow and achievement border */}
      <div
        ref={cardRef}
        onMouseEnter={handleAchievementHover}
        className={`rounded-lg border ${
          isAchievement ? 'border-purple-300 shadow-purple-100' : 'border-gray-200'
        ...
        <div className="p-6">
          ...
          <div className="flex items-start gap-2 mb-3">
            <div className="flex-1">
              <h2 className="text-lg font-semibold text-gray-900">{entry.entry_metadata.title}</h2>
            </div>
          ...
          {/* Achievement Box - Only for achievement entries */}
          {isAchievement && (
            <div className="bg-purple-50 border border-purple-200 rounded-lg p-4 mb-4">
              ...
              <p className="text-sm text-purple-700">Successful completion of critical WebSocket integration with real-time collaboration features</p>
            </div>
          )}
...
export default FeedAchievement;
