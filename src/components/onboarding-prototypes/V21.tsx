...
type Phase = 'question' | 'pick-tool' | 'analyzing' | 'timeline-preview' | 'stories-preview' | 'dashboard-preview' | 'gap-callout' | 'enhanced';
...
const PHASE_LIST: Phase[] = ['question', 'pick-tool', 'analyzing', 'timeline-preview', 'stories-preview', 'dashboard-preview', 'gap-callout', 'enhanced'];
...
function MiniTimelineSpine({ activities, toolId }: { activities: typeof mockActivities; toolId: ToolId }) {
  const toolActivities = activities.filter((a) => a.tool === toolId).slice(0, 5);
  return (
        ...
        <div key={i} className="flex items-start gap-2">
          <div className="flex flex-col items-center">
            <div className={`w-2 h-2 rounded-full shrink-0 mt-1 ${
              ...
              act.type.includes('Closed') ? 'bg-purple-400' : 'bg-gray-300'
            }`} />
            {i < toolActivities.length - 1 && <div className="w-px h-4 bg-gray-200 mt-0.5" />}
          </div>
...
function MiniSTARCard({ sections, evidence }: { sections: typeof mockStories[0]['sections']; evidence: Record<string, number> }) {
  return (
    <div className="space-y-2">
...
export function OnboardingV21() {
  ...
  useEffect(() => {
    ...
    const progInterval = setInterval(() => {
      ...
      if (prog >= 100) {
        ...
        setTimeout(() => goToPhase('timeline-preview'), 400);
      }
    }, SYNC_DELAY / 34);
  ...
  const currentEvidence = phase === 'enhanced' ? FULL_EVIDENCE : INITIAL_EVIDENCE;
  const phaseIdx = PHASE_LIST.indexOf(phase);
  const progressWidth = `${(phaseIdx / (PHASE_LIST.length - 1)) * 100}%`;
  const visibleGaps = EVIDENCE_GAPS.filter((g) => g.toolId !== primaryTool);
  const selectedTool = mockTools.find((t) => t.id === primaryTool);
  const SelectedToolIcon = primaryTool ? TOOL_ICONS[selectedTool?.icon ?? ''] : null;
    ...
    <div className="min-h-screen bg-gray-50 flex items-center justify-center p-6">
      <div className="w-full max-w-2xl">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-gray-500 mt-1 text-sm">We&apos;ll pull real evidence from your work tools and show you where everything lives.</p>
        </div>
        ...
        <div className={`transition-all duration-200 ease-out ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-2'}`}>
            ...
            <Card>
              <CardHeader>
                <div className="flex items-center gap-2">
                  ...
                  {SelectedToolIcon && <SelectedToolIcon size={14} className="text-gray-400" />}
                  <Badge variant="secondary" className="ml-auto text-xs">
                    {selectedTool?.activityCount ?? 0} activities
                  </Badge>
                </div>
              </CardHeader>
              <CardContent>
                <div className="grid grid-cols-[1fr,200px] gap-4">
                  ...
                  <div className="border rounded-lg p-3 bg-gray-50">
                    <MiniTimelineSpine activities={mockActivities} toolId={primaryTool!} />
                  </div>
                ...
                <div className="mt-4 flex items-center gap-2 px-3 py-2 bg-blue-50 border border-blue-200 rounded-lg">
                  ...
                  <p className="text-xs text-blue-700">Your activities appear here automatically as your tools sync.</p>
                </div>

                <button
                  onClick={() => goToPhase('stories-preview')}
                  className="w-full mt-4 flex items-center justify-center gap-2 py-2.5 px-4 bg-primary-600 text-white rounded-lg text-sm font-medium hover:bg-primary-700 transition-colors"
                >
                  Continue
          ...
          {/* Stories Preview */}
          {phase === 'stories-preview' && (
            <Card>
              <CardHeader>
                <div className="flex items-center gap-2">
                  <BookOpen size={16} className="text-primary-600" />
                  <CardTitle className="text-lg">Stories</CardTitle>
                  <Badge variant="secondary" className="text-xs">1 story</Badge>
                </div>
              </CardHeader>
              <CardContent>
                {/* Summary bar */}
                <div className="flex items-center gap-2 mb-4 px-3 py-2 bg-gray-50 rounded-lg border">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <span className="text-xs font-semibold text-amber-600 tabular-nums">{INITIAL_CONFIDENCE}%</span>
                  </div>
                </div>

                {/* STAR sections */}
                <MiniSTARCard sections={story.sections} evidence={INITIAL_EVIDENCE} />

                {/* Annotation */}
                <div className="mt-4 flex items-center gap-2 px-3 py-2 bg-blue-50 border border-blue-200 rounded-lg">
                  <div className="w-1.5 h-1.5 rounded-full bg-blue-400 shrink-0" />
                  <p className="text-xs text-blue-700">Your stories are built from your evidence — more tools means stronger stories.</p>
                </div>

                <button
                  onClick={() => goToPhase('dashboard-preview')}
                  className="w-full mt-4 flex items-center justify-center gap-2 py-2.5 px-4 bg-primary-600 text-white rounded-lg text-sm font-medium hover:bg-primary-700 transition-colors"
                >
                  Continue
                  <ChevronRight size={14} />
          ...
          {phase === 'dashboard-preview' && (
            <Card>
              ...
              <CardContent>
                {/* Roadmap */}
                <MiniRoadmapSVG completedCount={2} />

                {/* Integration health */}
                <div className="mt-4 border rounded-lg p-3">
                  <p className="text-xs font-semibold text-gray-700 mb-2">Integration Health</p>
                  <div className="flex items-center gap-3">
                    {SelectedToolIcon && <SelectedToolIcon size={14} className="text-green-500" />}
                    <span className="text-xs text-gray-600">{selectedTool?.name}</span>
                ...
                <div className="mt-4 flex items-center gap-2 px-3 py-2 bg-blue-50 border border-blue-200 rounded-lg">
                  <div className="w-1.5 h-1.5 rounded-full bg-blue-400 shrink-0" />
                  <p className="text-xs text-blue-700">Track your progress here — see what&apos;s connected and what&apos;s next.</p>
                </div>

                <button
                  onClick={() => goToPhase('gap-callout')}
                  className="w-full mt-4 flex items-center justify-center gap-2 py-2.5 px-4 bg-primary-600 text-white rounded-lg text-sm font-medium hover:bg-primary-700 transition-colors"
                >
                  See what&apos;s missing &rarr;
                </button>
              </CardContent>
            </Card>
          )}
          ...
          {/* Gap Callout */}
          {phase === 'gap-callout' && !isEnhancing && (
            <Card>
              <CardContent className="pt-6">
                <div className="rounded-lg border border-amber-200 bg-amber-50 p-4 border-l-4 border-l-amber-400">
                ...
                <CardHeader>
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm font-semibold tabular-nums text-green-600">{displayConfidence}% confidence</span>
                    </div>
                  </div>
                  <p className="text-sm text-gray-500">{story.title}</p>
                </CardHeader>
                <CardContent>
                  <MiniSTARCard sections={story.sections} evidence={currentEvidence} />
