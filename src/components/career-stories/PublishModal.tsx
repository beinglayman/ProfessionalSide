...
import { ClaimResolutionPanel, type UngroundedClaim } from './ClaimResolutionPanel';
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-w-3xl max-h-[85vh] overflow-hidden flex flex-col">
        <DialogHeader>
          <DialogTitle>
            {hasClaims ? 'Verify Your Claims' : 'Publish to Network'}
          </DialogTitle>
          <DialogDescription>
            {hasClaims
              ? `Your story has ${ungroundedClaims.length} claim${ungroundedClaims.length === 1 ? '' : 's'} that need evidence before publishing.`
        ...
        <div className="flex-1 overflow-y-auto">
          {hasClaims ? (
            /* Claim resolution step */
            <div className="p-1">
              <ClaimResolutionPanel
                claims={ungroundedClaims}
        ...
        <div className="flex justify-end gap-3 pt-4 border-t">
          ...
          <Button onClick={handlePublish} disabled={isPublishing || !!hasClaims}>
            {isPublishing
              ? 'Publishing...'
              : hasClaims
                ? `Publish (${ungroundedClaims.length} unverified)`
                : 'Publish to Network'}
          </Button>
        </div>
      </DialogContent>
