...
import { timelinePrototypes } from '../../components/timeline-prototypes';
import { cn } from '../../lib/utils';

type DesignId = string;

export function TimelinePrototypesPage() {
  const [activeDesign, setActiveDesign] = useState<DesignId>(timelinePrototypes[0].id);
  const currentDesign = timelinePrototypes.find((d) => d.id === activeDesign)!;
      ...
      <div className="mx-auto max-w-[1600px] px-4 py-8 sm:px-6">
        ...
        <div className="mb-6">
          <h1 className="text-2xl font-bold tracking-tight text-gray-900">
            Timeline Page Prototypes
          </h1>
          <p className="mt-1 text-sm text-gray-500">
            10 different design approaches for the Timeline page — click to switch
          </p>
        </div>

        {/* Design Switcher */}
        <div className="mb-8 flex flex-wrap gap-2">
          {timelinePrototypes.map((design) => {
