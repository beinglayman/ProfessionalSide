...
export default function ServiceStatusPageStandalone() {
  ...
  const checkServiceStatus = async (service: Service): Promise<ServiceStatus> => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:3002/api/services/status', {
        method: 'POST',
      ...
      console.log(`Response for ${service.id}:`, response.status, response.ok);
      
      if (response.ok) {
        const data = await response.json();
        console.log(`Data for ${service.id}:`, data);
        return {
          name: service.name,
          status: data.status,
          port: data.port,
        ...
        console.error(`HTTP error for ${service.id}:`, response.status, response.statusText);
      }
    } catch (error) {
  ...
  const executeServiceAction = async (service: Service, action: 'start' | 'stop' | 'restart') => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:3002/api/services/control', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ 
          serviceId: service.id,
      ...
      <div className="p-6 max-w-7xl mx-auto">
        <div className="flex justify-between items-center mb-6">
          <div>
            ...
            <p className="text-gray-600 text-sm mt-1">Monitor and manage your application services</p>
          </div>
        ...
        <div className="mt-8 bg-blue-50 border border-blue-200 rounded-lg p-4">
          <h3 className="font-medium text-blue-900 mb-2">Quick Commands</h3>
          <div className="text-sm text-blue-800 space-y-1">
            <div><code>./manage-services.sh status</code> - Check service status</div>
            <div><code>./manage-services.sh start</code> - Start all services</div>
            <div><code>./manage-services.sh restart backend</code> - Restart specific service</div>
            <div><code>./manage-services.sh logs backend</code> - View service logs</div>
          </div>
        </div>
      </div>
    </div>
