...
**CLI** (`npx ts-node backend/src/cli/oauth-cli.ts`): `status`, `inspect`, `refresh`, `validate-all`, `disconnect`, `simulate-failure`.
...
| `backend/src/cli/oauth-cli.ts` | OAuth CLI testing tool (6 commands) |
