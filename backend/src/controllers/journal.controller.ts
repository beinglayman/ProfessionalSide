...
/**
 * Clear all demo data (entries + activities) for the user.
 * Only works when X-Demo-Mode header is true.
 */
export const clearDemoData = asyncHandler(async (req: Request, res: Response): Promise<void> => {
  ...
  if (!(journalService as any).isDemoMode) {
    return void sendError(res, 'This endpoint requires X-Demo-Mode: true header', 400);
  }
