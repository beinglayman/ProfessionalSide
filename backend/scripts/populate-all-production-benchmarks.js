/**
 * Production Script: Populate ALL 973 Skills with Benchmarks
 * 
 * This script processes all skills in the database and generates benchmarks,
 * prioritizing by usage and category, with comprehensive email reporting.
 * 
 * PRODUCTION SAFETY:
 * - Batch processing (10 skills per batch)
 * - Rate limiting (2 minutes between batches)
 * - Resume capability (stores progress in database)
 * - Error handling (skips failures, continues processing)
 * - Email notification to honey72arora@gmail.com on completion
 */

const { PrismaClient } = require('@prisma/client');

// Use dynamic imports for TypeScript modules since we'll run with tsx
let SkillsBenchmarkService, EmailService;

async function loadServices() {
  try {
    const benchmarkModule = await import('../src/services/skills-benchmark.service.js');
    SkillsBenchmarkService = benchmarkModule.SkillsBenchmarkService;
...
let benchmarkService, emailService;
...
const CONFIG = {
  ...
  PROGRESS_TABLE: 'skill_benchmark_progress' // Virtual table for tracking
};
...
async function populateAllBenchmarks() {
  ...
  try {
    // Step 0: Load services
    console.log('📦 Loading services...');
    await loadServices();
    benchmarkService = new SkillsBenchmarkService();
...
async function processBatch(skills, batchNumber) {
  try {
    ...
    // Generate benchmarks for this batch
    processStats.apiCallsUsed++;
    const benchmarks = await benchmarkService.getSkillBenchmarks(skillNames);
    
    // Store results and update stats
    for (const skill of skills) {
      const benchmark = benchmarks.find(b => b.skillName === skill.name);
      const category = skill.category || 'Uncategorized';
      ...
      if (benchmark) {
        ...
        processStats.failedSkillsList.push({
          name: skill.name,
          category: category,
          error: 'No benchmark generated'
        });
        console.log(`     ❌ ${skill.name}: Failed to generate benchmark`);
      }
...
async function finalizeBenchmarkProcess() {
  ...
  console.log(`   📡 API calls used: ${processStats.apiCallsUsed}`);
  ...
  const failedSkillsHtml = processStats.failedSkillsList.length > 0
    ? `<h3>Failed Skills (${processStats.failedSkillsList.length})</h3>
       <ul style="max-height: 200px; overflow-y: auto;">
         ${processStats.failedSkillsList.map(skill => 
    ...
    <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
      ...
      <div style="background: #f5f5f5; padding: 20px; border-radius: 8px; margin: 20px 0;">
        ...
        <ul>
          ...
          <li><strong>Azure OpenAI API Calls:</strong> ${processStats.apiCallsUsed}</li>
      ...
      ${processStats.errors.length > 0 ? `
        <h3>⚠️ Errors Encountered (${processStats.errors.length})</h3>
        <ul>
          ${processStats.errors.map(error => 
            `<li><strong>${error.type}</strong>: ${error.message} (${error.timestamp.toLocaleString()})</li>`
          ).join('')}
        </ul>
      ` : '<p style="color: green;">✅ No errors encountered during processing!</p>'}
...
- API Calls Used: ${processStats.apiCallsUsed}
  ...
  const htmlContent = `
    ...
    <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
      ...
      <ul>
        ...
        <li><strong>API Calls Used:</strong> ${processStats.apiCallsUsed}</li>
      </ul>
      
      <p>The process can be resumed from where it left off. Please investigate the error and restart the script.</p>
