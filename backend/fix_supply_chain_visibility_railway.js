...
async function fixSupplyChainVisibilityRailway() {
  try {
    console.log('🔄 Adding Skills to Supply Chain Work Types for Visibility (RAILWAY)...\n');
    ...
    console.log('📊 RAILWAY SUPPLY CHAIN VISIBILITY FIX SUMMARY:');
    console.log(`✅ Skill mappings added: ${totalMappingsAdded}`);
    console.log(`❌ Skills not found: ${skillsNotFound}`);
    console.log('');

    // Verify the fix
    console.log('🔍 Verifying Railway Supply Chain Work Types Now Have Skills...');
    ...
    console.log(`\n✅ Updated Railway Supply Chain Work Types (${supplyChainWorkTypes.length}):`);
    ...
    console.log('\n🎉 Railway Supply Chain work types should now be visible in the UI!');
    console.log('   All work types now have at least 2 skills mapped.');

  } catch (error) {
    console.error('❌ Error fixing railway supply chain visibility:', error);
    throw error;
  } finally {
...
fixSupplyChainVisibilityRailway()
  .catch((error) => {
